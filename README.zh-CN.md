...
一个现代、易用、小体积、高性能、泛用型的纯 Rust 原生桌面 GUI 框架：信号驱动、控件式、面向跨平台。

> 译注：如果你会中文，请优先阅读本中文版本；这是作者的原稿，其他语言版本主要通过模型翻译，仅供参考。

> ⚠️ 稳定性声明：Flor 仍处于早期阶段。当前 API 只是初步稳定；在正式发布到 crates.io
> 之前，可能随时变更或移除，不会使用弃用标签做过渡。

Flor 是一个从零构建的控件式、响应式的 GUI 框架 —— 图形层、平台层、布局引擎集成、信号系统 —— 全部自研实现，不依赖 winit、wgpu
等重型库。设计初衷是创造一个作者自己理想中的 Rust GUI 框架：**小体积、高性能、易用的 API、零设计屏障**。

详细文档日后会以独立的网页项目发布，本 README 仅做框架概览和定位。

## 定位与目标

Flor 的定位是**泛用型、面向所有人的 Rust GUI 框架**：不为特定产品或垂直领域而生，目标是让任何想写原生 Rust
桌面应用的人第一时间想到它。API
追求低上手门槛；信号系统与句柄暴露方便多人协作与深度扩展；自研图形/平台层带来高性能与小体积。目前官方控件库与文档仍在打磨中，短期以核心框架体验为主，后续配合完善的控件库与文档，希望项目能开箱即“秀肌肉”。

适用场景：

- 小工具与桌面端工具 —— 小体积、快启动、原生外观
- 高性能可视化/交易类应用 —— 数据大屏、波形/行情等需要低延迟绘制
- 多人维护的纯 Rust 项目 —— 信号驱动的数据流易追踪、易重构，减少手动刷新和生命周期体操
- 重型 UI 且重性能场景 —— 控件式框架便于自定义控件，性能优于基于 diff tree 的组合式 UI
...
- **混合渲染模式（保留=即时基底）** —— 每个窗口可独立选择保留或即时模式；保留模式运行在即时模式执行路径上，性能几乎与即时模式持平（差距可忽略），并通过帧精度唤醒支持高性能动画。
...
- **框架与控件分离** —— 核心框架不包含控件，控件库独立维护；简单小游戏也能作为 `View` 实现。
  `View` 实现来写。
- **类似 TailwindCSS 的工具类** —— 支持用 `"flex flex-col gap-2 p-4"` 等熟悉的类名来定义布局与样式，让从 Web
  开发转来的开发者倍感亲切。

> 愿景：让 Flor 成为 Rust 生态中大家默认会想到、愿意尝试的 GUI 选项之一。当开发者需要原生桌面应用时，希望它能自然进入候选，就像
> C++ 的 Qt、C# 的 WPF 那样。
...
图形后端与平台层处于同一依赖层级，互不依赖，由核心框架按需选择。

| Crate                    | 用途                       | 依赖层级    |
|--------------------------|--------------------------|---------|
| `flor`                   | 核心框架 — 视图系统、信号、事件循环、窗口管理 | L2      |
| `flor-base`              | 共享类型和图形抽象                | L0 基础   |
...
- `direct2d` — Windows 原生 Direct2D GPU 后端
- `opengl` — 兼容性 GPU 后端
- `tiny-skia` — 纯 CPU 渲染后端

> Flor 同时支持 GPU 与 CPU 渲染：至少启用一个后端即可，最多可同时启用 1 个 GPU 后端 + 1 个 CPU 后端，GPU 初始化失败时会回退到
> CPU。
...
| Feature                        | 说明                                     |
|--------------------------------|----------------------------------------|
| `direct2d`                     | Direct2D GPU 渲染后端（启用 GPU 渲染链）          |
| `opengl`                       | OpenGL GPU 渲染后端                        |
| `tiny-skia`                    | 纯 CPU 渲染后端（启用 CPU 渲染链）                 |
| `cpu-render-backend`           | 显式选择 CPU 渲染链（`tiny-skia` 隐式开启）         |
| `gpu-render-backend`           | 显式选择 GPU 渲染链（`direct2d`/`opengl` 隐式开启） |
| `layout-flex`                  | Flexbox 布局（taffy/flexbox）              |
| `layout-grid`                  | Grid 布局（taffy/grid）                    |
| `layout-block`                 | Block 布局（taffy/block）                  |
| `class`                        | 类名支持（Tailwind 风格）                      |
| `svg`                          | 顶层 SVG 支持，透传到渲染后端                      |
| `clipboard`                    | 剪贴板                                    |
| `drag-drop`                    | 拖放                                     |
| `tray`                         | 系统托盘                                   |
| `memory-font`                  | 内存字体加载                                 |
