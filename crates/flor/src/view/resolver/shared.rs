...
pub fn parse_length_percentage_auto(
    ...
    if let Some(v) = value.strip_suffix("rem") {
        if let Ok(n) = v.parse::<f32>() {
            return Some(LengthPercentageAuto::Length(cfg.rem_to_px(n)));
        }
    }
    if let Some(v) = value.strip_suffix("pt") {
        if let Ok(n) = v.parse::<f32>() {
            return Some(LengthPercentageAuto::Length(cfg.pt_to_px(n)));
        }
    }
...
pub fn parse_dimension(value: &str, cfg: &UnitResolver) -> Option<Dimension> {
    ...
    if let Some(v) = value.strip_suffix("rem") {
        if let Ok(n) = v.parse::<f32>() {
            return Some(Dimension::Length(cfg.rem_to_px(n)));
        }
    }
    if let Some(v) = value.strip_suffix("pt") {
        if let Ok(n) = v.parse::<f32>() {
            return Some(Dimension::Length(cfg.pt_to_px(n)));
        }
    }
    if let Ok(n) = value.parse::<f32>() {
...
pub fn parse_length_percentage(value: &str, cfg: &UnitResolver) -> Option<LengthPercentage> {
    if value == "full" {
    ...
    if let Some(v) = value.strip_suffix("rem") {
        if let Ok(n) = v.parse::<f32>() {
            return Some(LengthPercentage::Length(cfg.rem_to_px(n)));
        }
    }
    if let Some(v) = value.strip_suffix("pt") {
        if let Ok(n) = v.parse::<f32>() {
            return Some(LengthPercentage::Length(cfg.pt_to_px(n)));
        }
    }
    if let Ok(n) = value.parse::<f32>() {
...
/// 解析简单长度值 (px, rem, 或纯数字)
#[inline]
pub fn parse_length(value: &str, rem_px: f32) -> Option<f32> {
    if let Some(v) = value.strip_suffix("px") {
